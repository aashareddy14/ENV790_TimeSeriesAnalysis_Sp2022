...
ep_plt <- electricity_price_processed[228:239,] %>%
  select(Month, All.sectors) %>%
  rename(observed = All.sectors)

ep_plt <- cbind(ep_plt, mean = MEAN_seas$mean, snaive = SNAIVE_seas$mean, sarima = SARIMA_for$mean)

ep_plt <- pivot_longer(ep_plt, 2:5)

#Use ggplot to show observed and forecast on a single plot.
ggplot(ep_plt, aes(x = Month, y = value, color = name)) + 
  geom_line()
